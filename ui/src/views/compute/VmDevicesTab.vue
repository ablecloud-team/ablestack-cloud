...
@cancel="close">
      ...
      <template v-if="dialog === 'allocate'">
        <a-form layout="vertical">
          <a-form-item :label="d('mode')"><a-select v-model:value="mode" :disabled="busy" @change="changeMode"><a-select-option value="existing">{{ d('existing') }}</a-select-option><a-select-option v-if="api('createVhbaDevice') && allowed('vhba')" value="create">{{ d('createVhba') }}</a-select-option></a-select></a-form-item>
          <a-form-item v-if="!vm.hostid" :label="$t('label.host')"><a-select v-model:value="hostId" :disabled="busy" @change="fetchCandidates"><a-select-option v-for="host in hosts" :key="host.id" :value="host.id">{{ host.name }}</a-select-option></a-select><p class="device-help">{{ d('hostHelp') }}</p></a-form-item>
          ...
          <a-checkbox v-model:checked="ack" :disabled="busy">{{ d('ack') }}</a-checkbox>
        </a-form>
      </template>
      <template v-else-if="selected">
        ...
        <template v-if="dialog === 'release'"><a-alert type="warning" show-icon :message="d(selected.devicetype === 'pci' ? 'pciWarning' : 'releaseWarning')" /><a-checkbox v-model:checked="ack" :disabled="busy">{{ d('ackRelease') }}</a-checkbox></template>
