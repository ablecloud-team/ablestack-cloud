...
:pagination="{ pageSize: 10, hideOnSinglePage: true }"
    ...
    <a-modal :visible="!!form" :title="dialogTitle" :width="760" centered wrap-class-name="vm-iso-modal" :mask-closable="false" @cancel="closeDialog">
      <a-alert v-if="listRefreshFailed" class="iso-spacing" type="error" show-icon :message="$t('message.list.refresh.stale')" />
      ...
      <template v-else-if="form === 'detach'">
        ...
        <a-checkbox-group v-model:value="detachIds" class="iso-candidates">
          <label v-for="iso in rows" :key="iso.id" class="iso-choice">
            <a-checkbox :value="iso.id" :disabled="busy || loading || !!reason(false)" />
            <span><strong>{{ name(iso) }}</strong><span class="iso-note">{{ $t('label.vmiso.device') }}: {{ slot(iso) }}</span></span>
          </label>
        </a-checkbox-group>
        <a-empty v-if="!loading && !rows.length" :description="$t('message.vmiso.empty')" />
      </template>
      ...
      <template #footer>
        <a-button @click="closeDialog">{{ $t(['attach', 'detach'].includes(form) ? 'label.cancel' : 'label.close') }}</a-button>
        ...
        <a-button v-if="form === 'detach'" type="primary" :disabled="busy || loading || !targets.length || !!reason(false)" @click="begin('detachIso', targets)">{{ $t('label.vmiso.detach.count', { count: targets.length }) }}</a-button>
...
export default {
  name: 'VmIsoManager',
  props: { resource: { type: Object, required: true }, dialogOnly: Boolean, currentAction: { type: Object, default: () => ({}) } },
  mixins: [listRefreshMixin(['fetchData'], { active: vm => !!vm.resource.id })],
  data () { return { vm: this.resource, mediaById: {}, loading: false, selected: [], search: '', form: '', candidates: [], candidatesLoading: false, candidateError: '', candidateSearch: '', attachIds: [], detachIds: [], detail: null, forced: false, candidateRequest: 0 } },
  computed: {
    ...
    targets () { return this.rows.filter(iso => this.detachIds.includes(iso.id)) },
  ...
  watch: {
    scopeKey () { this.vm = this.resource; this.mediaById = {}; this.closeDialog(); this.selected = []; this.candidateRequest++; this.fetchData() },
    resource (value) { this.vm = value },
    form () { this.candidateRequest++ }
  },
  created () { if (this.dialogOnly) this.openAction(); else this.fetchData(); this.onComplete = () => this.fetchData(); eventBus.on('async-job-complete', this.onComplete) },
  beforeUnmount () { eventBus.off('async-job-complete', this.onComplete); this.candidateRequest++ },
  methods: {
    closeDialog () { this.form = ''; if (this.dialogOnly) this.$emit('close-action') },
    async openAction () {
      if (this.busy) { this.form = 'progress'; return }
      if (this.currentAction.api === 'attachIso') await this.openAttach()
      else { this.openDetach([]); await this.fetchData() }
    },
    ...
    async openAttach () {
      ...
      try {
        ...
        if (this.listRefreshFailed) throw new Error('VM unavailable')
        const found = []
    ...
    openDetach (items) { this.detachIds = items.map(i => i.id); this.forced = false; this.form = 'detach' },
    begin (api, items) {
