...
## 검증 및 배포
- 실제 서비스 경로에서 RESUME 처리 성공 뒤 응답만 유실하는 fault injection으로 최소 3회 재시도, export 세대/PID/포트 유지 확인. VM 데이터 경로는 끊지 않는다.
...
- 4개 관리 서버 schema 선적용 및 변경 클래스 배포, 필요 시 qemu shell 파일만 배포. PR #1022/#56과 Epic #950 갱신.
