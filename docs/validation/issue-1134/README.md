...
- 관리 서버는 수정 클래스를 기존 패키지 JAR에 반영하고 백업을 보존했다. 추가 `core` 변경은 테스트 호스트 31-2에도 반영했다.
...
- 모든 장치 종류의 실제 할당이나 재시작 지속성을 검증한 것은 아니다. USB/PCI/물리 HBA 및 FC LUN은 적합한 독립 장치가 없어 실제 할당 결과를 보장하지 않는다.
